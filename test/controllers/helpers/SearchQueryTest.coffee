...
describe "Operations for Search Query Builder", ->
  ...
  it "should build search query for latitude, longitude, distance, tags", (done) ->
    ...
    q.should.eql {'geo':{$near:{ $geometry :{ type : "Point" ,coordinates : [ -93.5, 40.01 ]},$maxDistance : 25}}, 'tags' :{$in: ["ATTRACTIONS","AMUSEMENT","ARCADES"]}}
    done()
  ...
  it "should transform longitude/latitude parameter for search", (done) ->
    ...
    q.should.eql {'geo':{$near:{ $geometry :{ type : "Point" ,coordinates : [ -93.5, 40.01 ]},$maxDistance : 40234}}}
    done()

  it "should transform longitude/latitude parameter for search", (done) ->
    q = new SearchQuery().buildFromParams {ll:'-94.595033,39.102704'}
    q.should.eql {'geo':{$near:{ $geometry :{ type : "Point" ,coordinates : [ -94.595033,39.102704 ]},$maxDistance : 40234}}}
    done()    

  it "should build a query using ll and maxdistance", (done) ->
    q = new SearchQuery().buildFromParams {ll:'-94.595033,39.102704', radius: 10}
    q.should.eql {'geo':{$near:{ $geometry :{ type : "Point" ,coordinates : [ -94.595033,39.102704]}, $maxDistance : 10}}}
    done()
  ...
  it "should handle event tags correctly as well as location", (done) ->
    ...
    q.should.eql {'geo':{$near:{ $geometry :{ type : "Point" ,coordinates : [ -94.595033,39.102704]}, $maxDistance : 10}}, 'tags':{$in:["PETS","FOOD"]}}
    done()
  ...
  it "should handle maxdistance with float values and cost", (done) ->
    ...
    q.should.eql {'geo':{$near:{ $geometry :{ type : "Point" ,coordinates : [ 1.01, 1.01 ]}, $maxDistance : 10.1}}, 'maxCost':{$lte : 20.00}}
    done() 
  ...
  it "should handle maxdistance with float values and start date", (done) ->
    ...
    q.should.eql {'geo':{$near:{ $geometry :{ type : "Point" ,coordinates : [ 1.01, 1.01 ]}, $maxDistance : 10.1}}, 'occurrences':{$gte : start}}
    done()  
  ...
  it "should handle maxdistance with float values and both dates", (done) ->
    ...
    q.should.eql {'geo':{$near:{ $geometry :{ type : "Point" ,coordinates : [ 1.01, 1.01 ]}, $maxDistance : 10.1}}, 'occurrences':{$gte : start, $lte :end}}
    done()              
